...
def main_encode(filepath_read,filepath_write):
    huf_obj = Huffman()
    file_obj = File()
    '''Чтение исходного файла'''
    arr_file_data, _, flag  = file_obj.get_binary_data(filepath_read)
    '''Кодирование методом Хаффмана'''
    # code_str, code,_ = huf_obj.get_code(arr_file_data) Бирючков
    code_str, code, dict_2 = huf_obj.get_code(arr_file_data)
    encode = huf_obj.encode(arr_file_data, code)
    '''Запись закодированных данных в файл'''
    file_obj.write_encode_and_tree(filepath_write,code,encode,flag)
    print("Данные записаны в файл ",filepath_write)
    return dict_2,len(arr_file_data)
...
if __name__ == '__main__':
    ...
    main_encode(filepath_read,filepath_write)
